   ...
   //                outputPath, prefixes[i], chiN, r_swap, release_flag, VERSION);
    ...
    for (t = 0; t < tmax; ++t) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            //int    chain = i / N;
            attempted++; 
            double U_bond_old = bondEnergy((i*N+j), N, beadX, beadY, juncX, juncY,
                                           Jbond, nj, b2, b2a, Lx, Ly, dim);
            gridPointInterpolator oldS = makeInterpolator(beadX[i*N+j], beadY[i*N+j], dL, Mx, My);
...
//Ufield[oldS.idx[m]] * pref;
            ...
            double U_old = U_bond_old + U_field_old*(double)pref;

            // trial move
            double x_old = beadX[i*N+j], y_old = beadY[i*N+j];
            beadX[i*N+j] += 0.05 * gasdev(&idum_thread);
            beadY[i*N+j] += 0.05 * gasdev(&idum_thread);
            wrap2D(&beadX[i*N+j], &beadY[i*N+j], Lx, Ly);

            // new bond + new field
            double U_bond_new = bondEnergy((i*N+j), N, beadX, beadY, juncX, juncY, // calling bondEnergy with updated positions
                                           Jbond, nj, b2, b2a, Lx, Ly, dim);
            gridPointInterpolator newS = makeInterpolator(beadX[i*N+j], beadY[i*N+j], dL, Mx, My); //interpolater with new positions
            ...
            double dU = (U_bond_new + U_field_new*(double)pref) - U_old; //Change in energy is our new energy state minus the previous one
