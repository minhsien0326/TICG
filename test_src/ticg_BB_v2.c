...
- Instead of using a fixed number of chains and calculating the corresponding sqrt_N_bar, 
  we now use a fixed sqrt_N_bar and calculate the number of chains accordingly. 
- Fix bug: DELTA_L and MG are dependent. 
------------- HISTORY ------------------------------------------------------------------------------
Version 1.0: 
This is the test version of the code, which is a 3D implementation of the binary blend system with TICG-MC framework. 
*/ 
...
#define SQRT_N_BAR   32                    // invariant degree of polymerisation, \sqrt{\bar{N}} = \rho_0 R_e^3 / N
#define N_BEAD       10                     // degree of polymerisation (number beads per chain) 
#define BOX_LENGTH   5.0                   // simulation box length (in unit of Re) 
...
// ---------- derived constants ------------------------------------------------
static const int     MG = (int) (BOX_LENGTH / DELTA_L);                                             // number of grid points along one dimension
static const int     NGRID = (MG*MG*MG);                                                              // total number of grid points 
static const double  INV_CLOUD_VOL = 1 / (DELTA_L * DELTA_L * DELTA_L);                             // cache the normalization factor for density cloud volume
static const int     N_CHAIN_TOT = (int) (BOX_LENGTH * BOX_LENGTH * BOX_LENGTH * SQRT_N_BAR);       // total number of chains in the system
static const double  dphi = (double) NGRID / (double)(N_CHAIN_TOT * N_BEAD);                        // density increment per bead 
/*
...
static void init_sys(void) {
    ...
    int N_CHAIN = (int) (N_CHAIN_TOT / N_TYPE); // number of chains per type 
...
int main(int argc, const char *argv[]) {
    ...
    if (files[0]) {
        ...
        fprintf(files[0], "N_CHAIN: %d\n", (int) N_CHAIN_TOT / N_TYPE);
        fprintf(files[0], "dphi"": %lf\n", dphi);
        fclose(files[0]);
    }
